  ...
  return <div className="calendar-page">
    ...
    <section className="card selected-day-card">
      ...
      <div className="selected-day-list">
        ...
        {selectedTasks.map((task) => <article key={task.id} className="selected-day-item task-day-item"><div className="calendar-date"><strong>משימה</strong><span>{task.status}</span></div><div><strong>{task.title}</strong><small>{task.startDate === selectedDate ? 'תאריך התחלה' : task.dueDate === selectedDate ? 'תאריך יעד' : 'מועד מעקב / סיום'}{task.emailTo ? ` · ${task.emailTo}` : ''}</small><div className="context-links">{task.projectId && onProject && <button type="button" onClick={() => onProject(task.projectId!)}>פרויקט: {workspace.projects.find((project) => project.id === task.projectId)?.address || workspace.projects.find((project) => project.id === task.projectId)?.name || 'פתיחה'}</button>}{onTask && <button type="button" onClick={() => onTask(task.id)}>פתיחת המשימה</button>}</div></div><Chip tone={['בוצע', 'סגור'].includes(task.status) ? 'good' : 'brand'}>{task.status}</Chip></article>)}
