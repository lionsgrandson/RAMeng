...
function ClientDirectory({ workspace, setWorkspace, onSelectClient, canEdit, actor }: Props) {
  ...
  return <section className="clients-center">
    <div className="client-directory-head">
      <div>
        <h2>לקוחות ולידים</h2>
        <p>כל לקוח הוא נקודת כניסה אחת לכל הפרויקטים, המשימות, התקשורת והמסמכים שלו.</p>
      </div>
      {canEdit && <button className="primary" onClick={() => setAdding(true)}><Plus /> לקוח חדש</button>}
    </div>
    <div className="client-directory-search search-box"><Search /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="חיפוש לפי שם, חברה, טלפון או מייל..." /></div>
...
function ClientWorkspace({ workspace, setWorkspace, onSelectClient, onProject, canEdit, isAdmin, actor, clientId }: Props & { clientId: string }) {
  ...
  const tabs: { id: ClientTab; label: string }[] = [
    ...
    ...(isAdmin ? [{ id: 'activity' as ClientTab, label: 'פעילות ולוגים' }] : []),
  ]
  ...
  return <section className="client-workspace">
    <header className="client-workspace-header card">
      <button className="back-button" onClick={() => onSelectClient(null)}><ArrowRight /> כל הלקוחות</button>
      <div className="client-title-row">
        <div className="client-identity"><span className="avatar large">{contact.name.slice(0, 2)}</span><div><span className="eyebrow">כרטיס לקוח</span><h1>{contact.name}</h1><p>{contact.company || 'לקוח פרטי'} · {contact.phone || 'ללא טלפון'} · {contact.email || 'ללא מייל'}</p></div></div>
        <div className="client-header-actions"><Chip tone={contact.status === 'פעיל' ? 'good' : contact.status === 'ליד' ? 'brand' : 'neutral'}>{contact.status}</Chip>{canEdit && <button className="secondary" onClick={() => setEditingClient(true)}><Settings2 /> עריכת לקוח</button>}</div>
      </div>
      {!canEdit && <div className="read-only-banner">מצב צפייה בלבד — ניתן לראות את כל המידע, אך לא לשנות אותו.</div>}
    ...
    {tab === 'overview' && <div className="client-overview-grid">
      <section className="card client-metrics">
        ...
        <article><small>משימות פתוחות</small><strong>{openTasks.length}</strong><span>{openTasks.filter((task) => task.priority === 'דחופה' || task.status === 'דורש מעקב').length} דורשות טיפול</span></article>
        ...
        <article><small>מסמכים</small><strong>{files.length}</strong><span>{quotes.length} מסמכים כספיים</span></article>
      </section>
      <section className="card"><div className="card-head"><div><h2>פרטי קשר</h2><p>כל המידע המרכזי בלי לצאת מכרטיס הלקוח.</p></div>{canEdit && <button className="secondary" onClick={() => setEditingClient(true)}>עריכה</button>}</div><div className="client-detail-list"><div><span>טלפון</span><strong>{contact.phone || '—'}</strong></div><div><span>מייל</span><strong>{contact.email || '—'}</strong></div><div><span>חברה</span><strong>{contact.company || '—'}</strong></div><div><span>תגיות</span><strong>{contact.tags.length ? contact.tags.join(', ') : '—'}</strong></div></div>{contact.notes && <div className="client-notes-block"><strong>הערות</strong><p>{contact.notes}</p></div>}</section>
      <section className="card"><div className="card-head"><div><h2>פרויקטים אחרונים</h2><p>גישה מהירה לפרויקטים המקושרים ללקוח.</p></div>{canEdit && <button className="primary" onClick={() => setAddingProject(true)}><Plus /> פרויקט</button>}</div><div className="compact-project-list">{projects.slice(0, 5).map((project) => <button key={project.id} onClick={() => onProject(project.id)}><div><strong>{project.name}</strong><small>{project.address || 'ללא כתובת'}</small></div><Chip tone={project.status === 'בביצוע' ? 'good' : 'brand'}>{project.status}</Chip></button>)}{!projects.length && <EmptyState title="אין פרויקטים ללקוח" text="ניתן ליצור פרויקט ישירות מכרטיס הלקוח." />}</div></section>
      <section className="card"><div className="card-head"><div><h2>מה דורש טיפול</h2><p>משימות פתוחות ועדכונים אחרונים.</p></div></div><div className="compact-task-list">{openTasks.slice(0, 6).map((task) => <div key={task.id}><div><strong>{task.title}</strong><small>{workspace.projects.find((project) => project.id === task.projectId)?.name || ''}</small></div><Chip tone={task.priority === 'דחופה' || task.status === 'דורש מעקב' ? 'bad' : 'brand'}>{task.status}</Chip></div>)}{!openTasks.length && <div className="table-empty">אין משימות פתוחות.</div>}</div></section>
    </div>}

    {tab === 'projects' && <section className="card"><div className="card-head"><div><h2>פרויקטים</h2><p>צפייה, עריכה וכניסה מלאה לכל פרויקט של הלקוח.</p></div>{canEdit && <button className="primary" onClick={() => setAddingProject(true)}><Plus /> פרויקט חדש</button>}</div><div className="client-project-list">{projects.map((project) => <article key={project.id}>
      <div className="client-project-main"><div><strong>{project.name}</strong><small>{project.address || 'ללא כתובת'}</small></div><button className="secondary" onClick={() => onProject(project.id)}><FolderKanban /> פתיחת פרויקט</button></div>
    ...
    {tab === 'tasks' && <section className="card"><div className="card-head"><div><h2>משימות</h2><p>כל המשימות מכל הפרויקטים של הלקוח במקום אחד.</p></div>{canEdit && projects.length > 0 && <button className="primary" onClick={() => setAddingTask(true)}><Plus /> משימה חדשה</button>}</div><div className="client-task-table">{tasks.map((task) => <article key={task.id}>
    ...
    {tab === 'timeline' && <section className="card"><div className="card-head"><div><h2>ציר זמן</h2><p>עדכונים פנימיים, פגישות ומועדי מעקב בסדר כרונולוגי.</p></div>{canEdit && <button className="primary" onClick={() => setAddingNoteKind('timeline')}><Plus /> עדכון לציר הזמן</button>}</div><div className="client-timeline">{timeline.map((item) => <article key={`${item.type}-${item.id}`}><span className={`timeline-dot ${item.type}`} /><div><small>{dateTimeLabel(item.at)}</small><strong>{item.title}</strong><p>{item.text}</p>{item.type === 'note' && canEdit && <div className="timeline-actions"><button className="text-button" onClick={() => setEditingNote(item.note)}>עריכה</button><button className="text-button danger-text" onClick={() => removeNote(item.note)}>מחיקה</button></div>}</div></article>)}{!timeline.length && <EmptyState title="ציר הזמן ריק" text="הוסיפו עדכון או צרו משימות ואירועים בפרויקטים." />}</div></section>}

    {tab === 'messages' && <div className="client-message-grid">
      <section className="card"><div className="card-head"><div><h2>הודעות פנימיות</h2><p>הערות לצוות בלבד, מקושרות ללקוח ולפרויקט לפי הצורך.</p></div>{canEdit && <button className="primary" onClick={() => setAddingNoteKind('message')}><Plus /> הודעה פנימית</button>}</div><div className="internal-message-list">{notes.filter((note) => note.kind === 'message').map((note) => <article key={note.id}><header><strong>{note.createdBy}</strong><span>{dateTimeLabel(note.createdAt)}</span></header><p>{note.body}</p>{note.projectId && <small>{workspace.projects.find((project) => project.id === note.projectId)?.name}</small>}{canEdit && <footer><button className="text-button" onClick={() => setEditingNote(note)}>עריכה</button><button className="text-button danger-text" onClick={() => removeNote(note)}>מחיקה</button></footer>}</article>)}{!notes.some((note) => note.kind === 'message') && <div className="table-empty">אין עדיין הודעות פנימיות.</div>}</div></section>
      <section className="card"><div className="card-head"><div><h2>תקשורת מייל</h2><p>כל המשימות עם כתובת מייל או Gmail thread בפרויקטים של הלקוח.</p></div></div><div className="communication-list">{tasks.filter((task) => task.emailTo || task.gmailThreadId).map((task) => <article key={task.id}><div><Mail /><span><strong>{task.title}</strong><small>{task.emailTo || 'ללא כתובת'}{task.gmailThreadId ? ' · Thread מחובר' : ''}</small></span></div>{task.projectId && <button className="secondary" onClick={() => onProject(task.projectId!)}>פתיחה בפרויקט</button>}</article>)}{!tasks.some((task) => task.emailTo || task.gmailThreadId) && <div className="table-empty">אין התכתבויות מקושרות כרגע.</div>}</div></section>
    </div>}

    {tab === 'files' && <section className="card"><div className="card-head"><div><h2>קבצים ומסמכים</h2><p>כל הקבצים מכל הפרויקטים של הלקוח.</p></div></div><div className="client-file-list">{files.map((file) => <a key={file.id} href={file.url} target="_blank" rel="noreferrer"><span className="file-icon"><FileText /></span><div><strong>{file.name}</strong><small>{workspace.projects.find((project) => project.id === file.projectId)?.name || ''} · {dateLabel(file.uploadedAt)}</small></div><ExternalLink /></a>)}{!files.length && <EmptyState title="אין קבצים" text="קבצים שיועלו לפרויקטים של הלקוח יופיעו כאן אוטומטית." />}</div></section>}

    {tab === 'finance' && <section className="card"><div className="card-head"><div><h2>כספים</h2><p>הצעות מחיר, חיובים וסטטוס תשלום הקשורים ללקוח או לפרויקטים שלו.</p></div></div><div className="client-finance-list">{quotes.map((quote) => <article key={quote.id}><div><strong>{quote.title}</strong><small>{quote.number} · {workspace.projects.find((project) => project.id === quote.projectId)?.name || contact.name}</small></div><b>{money(quote.amount)}</b>{canEdit ? <input type="number" min="0" value={quote.paidAmount} onChange={(e) => setWorkspace((current) => ({ ...current, quotes: current.quotes.map((item) => item.id === quote.id ? { ...item, paidAmount: Number(e.target.value) } : item) }))} /> : <span>שולם {money(quote.paidAmount)}</span>}{canEdit ? <select value={quote.status} onChange={(e) => setWorkspace((current) => ({ ...current, quotes: current.quotes.map((item) => item.id === quote.id ? { ...item, status: e.target.value as typeof quote.status } : item) }))}><option>טיוטה</option><option>נשלחה</option><option>אושרה</option><option>נדחתה</option><option>שולמה חלקית</option><option>שולמה</option></select> : <Chip tone={quote.status === 'שולמה' ? 'good' : 'brand'}>{quote.status}</Chip>}</article>)}{!quotes.length && <EmptyState title="אין מסמכים כספיים" text="מסמכים שמקושרים ללקוח או לפרויקט שלו יופיעו כאן." />}</div></section>}

    {tab === 'activity' && isAdmin && <section className="card"><div className="card-head"><div><h2>פעילות ולוגים</h2><p>תיעוד פעולות שנרשמו על הלקוח, הפרויקטים והמשימות שלו.</p></div></div><div className="client-audit-list">{clientAudit.map((entry) => <article key={entry.id}><span><ListChecks /></span><div><strong>{entry.action}</strong><small>{entry.actor} · {entry.entity} · {dateTimeLabel(entry.at)}</small></div></article>)}{!clientAudit.length && <div className="table-empty">אין עדיין רשומות פעילות ללקוח הזה.</div>}</div></section>}
