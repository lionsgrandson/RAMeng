...
export default function SettingsPage({ workspace, setWorkspace }: { workspace: Workspace; setWorkspace: React.Dispatch<React.SetStateAction<Workspace>> }) {
  ...
  return <div className="settings-layout">
    <aside className="settings-nav card" role="tablist" aria-label="הגדרות">
      <button type="button" role="tab" aria-selected={tab === 'organization'} className={tab === 'organization' ? 'active' : ''} onClick={() => setTab('organization')}>חברה</button>
      <button type="button" role="tab" aria-selected={tab === 'integrations'} className={tab === 'integrations' ? 'active' : ''} onClick={() => setTab('integrations')}>חיבורים</button>
    </aside>
    <main>
...
function OrganizationSettings({ workspace, setWorkspace }: { workspace: Workspace; setWorkspace: React.Dispatch<React.SetStateAction<Workspace>> }) {
  ...
  return <section className="card settings-card">
    ...
    <div className="settings-form">
      ...
      <Field label="טלפון"><input type="tel" autoComplete="tel" value={settings.phone} onChange={(e) => patch('phone', e.target.value)} /></Field>
      <Field label="מייל"><input type="email" autoComplete="email" value={settings.email} onChange={(e) => patch('email', e.target.value)} /></Field>
      <Field label="מייל נוסף"><input type="email" value={settings.secondaryEmail} onChange={(e) => patch('secondaryEmail', e.target.value)} /></Field>
      <Field label="אתר"><input type="url" inputMode="url" value={settings.website} onChange={(e) => patch('website', e.target.value)} /></Field>
      <Field label="כתובת לוגו"><input type="url" inputMode="url" value={settings.logoUrl} onChange={(e) => patch('logoUrl', e.target.value)} /></Field>
    </div>
  </section>
}
...
function IntegrationSettings({ workspace, setWorkspace }: { workspace: Workspace; setWorkspace: React.Dispatch<React.SetStateAction<Workspace>> }) {
  ...
  return <section className="card settings-card">
    <div className="card-head"><h2>חיבורים</h2><button type="button" className="secondary" onClick={() => void refresh()}><RefreshCw /> רענון</button></div>
    ...
    <div className="integration-health">
      ...
      <article><span className="google-g">G</span><div><strong>Google</strong><span>{status?.google.email || 'Gmail · Calendar · Drive'}</span></div><Chip tone={status?.google.connected ? 'good' : 'warn'}>{status?.google.connected ? 'מחובר' : 'לא מחובר'}</Chip><button type="button" className="secondary" onClick={() => void connectGoogle()}>{status?.google.connected ? 'חיבור מחדש' : 'חיבור Google'}</button></article>
    </div>
