  ...
  return <>
    ...
    <div className="project-card-grid">{filteredProjects.map((project) => {
      ...
      return <button className={`project-card card ${missingAddress ? 'missing-address' : ''}`} key={project.id} onClick={() => onOpen(project.id)}>
        ...
        <span className="project-site-label">אתר / כתובת</span>
        <h2>{project.address || 'חסרה כתובת'}</h2>
        <p>{project.name || 'פרויקט בנייה'}</p>
    ...
    {canEdit && adding && <Modal title="פרויקט חדש" onClose={() => setAdding(false)} wide><form className="form-grid two-col" onSubmit={submit}>
      ...
      <Field label="כתובת *" hint="שדה חובה — הכתובת היא המזהה הראשי של אתר הבנייה"><input name="address" required aria-required="true" placeholder="רחוב, מספר, עיר" /></Field>
      ...
      <Field label="לקוחות"><select name="clientIds" multiple size={4}>{workspace.contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.name}</option>)}</select></Field>
...
export function ProjectWorkspace({ projectId, workspace, setWorkspace, orgId, onBack, onClient, onTabChange, initialTab = 'summary', canEditProject = true, canEditTasks = true, canEditCalendar = true, canEditFiles = true, canEditReports = true, canEditMail = true, projectPermissions, taskPermissions, calendarPermissions, filePermissions, reportPermissions, communicationPermissions }: { projectId: string; workspace: Workspace; setWorkspace: React.Dispatch<React.SetStateAction<Workspace>>; orgId: string; onBack: () => void; onClient?: (id: string) => void; onTabChange?: (tab: ProjectTab) => void; initialTab?: ProjectTab; canEditProject?: boolean; canEditTasks?: boolean; canEditCalendar?: boolean; canEditFiles?: boolean; canEditReports?: boolean; canEditMail?: boolean; projectPermissions?: AreaPermissions; taskPermissions?: AreaPermissions; calendarPermissions?: AreaPermissions; filePermissions?: AreaPermissions; reportPermissions?: AreaPermissions; communicationPermissions?: AreaPermissions }) {
  ...
  const tabs: [ProjectTab, string][] = ([['summary', 'סקירה'], ['tasks', 'משימות'], ['mail', 'מייל'], ['calendar', 'יומן'], ['drive', 'דרייב'], ['reports', 'דוחות'], ['files', 'קבצים']] as [ProjectTab, string][]).filter(([id]) => tabAllowed(id))
  const selectedClients = new Set(project.clientIds)
  return <div className="project-workspace"><div className="project-header card"><button type="button" className="back-button" onClick={onBack}><ArrowRight /> פרויקטים</button><div className="project-title"><div><span className="project-site-label">אתר / כתובת</span><h1>{project.address || project.name || 'ללא כתובת'}</h1><p>{project.name || 'פרויקט בנייה'} · מנהל: {workspace.team.find((member) => member.id === project.managerId)?.name || 'לא משויך'}</p></div>{(canUpdateProject || canStatusProject) && <div className="project-edit-inline">{canUpdateProject && <button type="button" className="secondary" aria-expanded={editingDetails} onClick={() => setEditingDetails((value) => !value)}><Settings2 /> עריכה</button>}{canStatusProject && <label className="inline-control-label"><span>סטטוס</span><select value={project.status} onChange={(e) => patchProject({ status: e.target.value as ProjectStatus })}><option>בתכנון</option><option>בביצוע</option><option>בהמתנה</option><option>הושלם</option><option>מוקפא</option></select></label>}{canUpdateProject && <label className="inline-control-label"><span>התקדמות</span><div className="progress-input"><input type="number" min="0" max="100" value={project.progress} onChange={(e) => patchProject({ progress: Math.max(0, Math.min(100, Number(e.target.value))) })} /><b>%</b></div></label>}</div>}</div><nav className="project-tabs" role="tablist" aria-label="חלקי הפרויקט">{tabs.map(([id, label]) => <button type="button" role="tab" aria-selected={tab === id} key={id} className={tab === id ? 'active' : ''} onClick={() => selectTab(id)}>{label}</button>)}</nav></div>{!project.address.trim() && <div className="missing-address-banner" role="alert"><strong>לפרויקט הזה חסרה כתובת.</strong><span>כתובת היא שדה חובה ומשמשת כמזהה הראשי של אתר הבנייה. יש לעדכן אותה בפרטי הפרויקט.</span></div>}{project.clientIds.length > 0 && <div className="context-links project-client-links"><span>לקוחות:</span>{project.clientIds.map((id) => { const client = workspace.contacts.find((item) => item.id === id); return client && <button type="button" key={id} onClick={() => onClient?.(id)}>{client.name}</button> })}</div>}
    {canUpdateProject && editingDetails && <section className="card settings-card"><div className="card-head"><h2>פרטי הפרויקט</h2><button type="button" className="secondary" onClick={() => setEditingDetails(false)}>סגירה</button></div><div className="settings-form"><Field label="שם הפרויקט"><input value={project.name} onChange={(e) => patchProject({ name: e.target.value })} /></Field><Field label="כתובת *" hint="כתובת חובה לכל פרויקט"><input required aria-required="true" aria-invalid={!project.address.trim()} value={project.address} onChange={(e) => patchProject({ address: e.target.value })} placeholder="רחוב, מספר, עיר" /></Field><Field label="מנהל פרויקט"><select value={project.managerId || ''} onChange={(e) => patchProject({ managerId: e.target.value || undefined })}><option value="">לא משויך</option>{workspace.team.filter((member) => member.active).map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field><Field label="תאריך התחלה"><input type="date" value={project.startDate || ''} onChange={(e) => patchProject({ startDate: e.target.value || undefined })} /></Field><Field label="תאריך יעד"><input type="date" value={project.targetDate || ''} onChange={(e) => patchProject({ targetDate: e.target.value || undefined })} /></Field><Field label="לקוחות משויכים" hint="אפשר לבחור מספר לקוחות"><select multiple size={Math.min(6, Math.max(3, workspace.contacts.length))} value={project.clientIds} onChange={(e) => patchProject({ clientIds: Array.from(e.currentTarget.selectedOptions).map((option) => option.value) })}>{workspace.contacts.map((contact) => <option key={contact.id} value={contact.id}>{selectedClients.has(contact.id) ? '✓ ' : ''}{contact.name}</option>)}</select></Field><Field label="הערות"><textarea rows={5} value={project.notes || ''} onChange={(e) => patchProject({ notes: e.target.value })} /></Field></div></section>}
    {tab === 'summary' && <div className="project-summary"><div className="metric-grid">{taskAccess.view && <button type="button" className="metric card metric-link" onClick={() => selectTab('tasks')}><div><small>משימות</small><strong>{open.length}</strong><span className="metric-note">{attention.length} לטיפול</span></div></button>}{reportAccess.view && <button type="button" className="metric card metric-link" onClick={() => selectTab('reports')}><div><small>פיקוח</small><strong>{openReportItems.length}</strong><span className="metric-note">{reports.length} דוחות</span></div></button>}{calendarAccess.view && <button type="button" className="metric card metric-link" onClick={() => selectTab('calendar')}><div><small>יומן</small><strong>{events.length}</strong><span className="metric-note">{events.filter((event) => new Date(event.start).getTime() > Date.now()).length} קרובים</span></div></button>}{fileAccess.view && <button type="button" className="metric card metric-link" onClick={() => selectTab('files')}><div><small>קבצים</small><strong>{files.length}</strong><span className="metric-note">{project.driveFolderId ? 'Drive מחובר' : 'ללא Drive'}</span></div></button>}</div>{project.notes && <section className="card"><div className="card-head"><h2>הערות</h2>{canUpdateProject && <button type="button" className="secondary" onClick={() => setEditingDetails(true)}>עריכה</button>}</div><div className="card-body"><p className="project-notes">{project.notes}</p></div></section>}{taskAccess.view && <section className="card"><div className="card-head"><h2>פתוח</h2></div><div className="summary-open-list">{open.slice(0, 12).map((task) => <button key={task.id} onClick={() => { setFocusedTaskId(task.id); selectTab('tasks') }}><div><strong>{task.title}</strong><small>{workspace.team.find((member) => member.id === task.assigneeId)?.name || 'ללא אחראי'} · התחלה {dateLabel(task.startDate)}</small></div><Chip tone={task.status === 'דורש מעקב' || task.priority === 'דחופה' ? 'bad' : 'brand'}>{task.status}</Chip><span>{dateLabel(task.followUpDate || task.dueDate)}</span></button>)}{!open.length && <EmptyState title="אין משימות פתוחות" text="הפרויקט מעודכן ואין כרגע משימות פתוחות." />}</div></section>}</div>}
...
function ProjectCalendar({ project, workspace, setWorkspace, canEdit }: { project: Project; workspace: Workspace; setWorkspace: React.Dispatch<React.SetStateAction<Workspace>>; canEdit: boolean }) {
  ...
  return <section className="card"><div className="card-head"><h2>יומן</h2>{canEdit && <div className="inline-create"><label className="inline-control-label"><span>משימה לתזכורת</span><select aria-label="בחירת משימה לתזכורת" value={addingTask} onChange={(e) => setAddingTask(e.target.value)}><option value="">בחירת משימה</option>{workspace.tasks.filter((task) => task.projectId === project.id).map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}</select></label><button type="button" className="secondary" disabled={!addingTask} onClick={() => void createFromTask()}><CalendarDays /> יצירה ביומן</button></div>}</div>{error && <div className="error-banner">{error}</div>}<div className="calendar-list">{events.map((event) => <article key={event.id}><div className="calendar-date"><strong>{new Date(event.start).getDate()}</strong><span>{new Intl.DateTimeFormat('he-IL', { month: 'short' }).format(new Date(event.start))}</span></div><div><strong>{event.title}</strong><small>{dateTimeLabel(event.start)}{event.location ? ` · ${event.location}` : ''}</small></div>{event.googleHtmlLink && <a className="icon-btn" href={event.googleHtmlLink} target="_blank" rel="noreferrer" aria-label={`פתיחת ${event.title} ב-Google Calendar`}><ExternalLink /></a>}</article>)}{!events.length && <EmptyState title="אין אירועים בפרויקט" text="ניתן ליצור פגישה או תזכורת ישירות ממשימה." />}</div></section>
}
